...
int buscar_palabra(Hash *hash , char *palabra)
{
	...
	else
	{
		...
		while(hash[indice].arreglo!=NULL)//si hay algo en ese indice de hash se buscara palabra de lo contrario no esta
		{
			...
			strcpy( nombre1,hash[indice].arreglo->nombre  );
			strcpy( nombre2,palabra  );
		//	nombre1=malloc(sizeof(nombre1)*15);
		//	nombre2=malloc(sizeof(nombre2)*15);
			printf("nombre en arregloooaaaaa: %s \n palabra a compararbbbbb :%s \n",nombre1,nombre2);
